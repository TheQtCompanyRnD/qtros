...
if req_needs_wrap:
    ...
    req_include = f'<{package_name}/srv/{to_snake_case(request_msg.structure.namespaced_type.name)}.hpp>'
    ...
    if req_field_info:
        ...
        include_target = f'qtros2_{pkg}' if is_cross else package_name
...
if resp_needs_wrap:
    ...
    resp_include = f'<{package_name}/srv/{to_snake_case(response_msg.structure.namespaced_type.name)}.hpp>'
...
class @(qt_class_name)ServiceClient : public QRos2ServiceClientBase
{
    Q_OBJECT
    QML_ELEMENT
